...
"""
print("******week_report_math_route开始运行******")
math_star = wc.end - dt.timedelta(days=7)
end = wc.end


groupby_status =["route_id","route_name","operation_center_id","operation_center_name",]
# 选取上周数据表
rides_df_7 = wc.rides_df.query(f"serving_at >= @math_star and serving_at < @wc.end")  # 上周数据表，用作计算

# 没有状态筛选
null_status = rides_df_7.groupby(groupby_status).agg(
...
null_status["accepted_order_A"]=rides_df_7.query("manual_assigned_count >0 and accepted_at.isnull()").groupby(groupby_status).\
...
#   完成订单状态
completed_status = rides_df_7.query("actual_dropoff_at.notnull()").groupby(groupby_status).agg(
...
print("####" * 15)
order_paytime_mean = rides_df_7.groupby(groupby_status).agg(
...
# 33 【响应时长min】
rides_df_7["respond_time"] = (rides_df_7["accepted_at"] - rides_df_7["created_at"]). \
    apply(lambda x: x.total_seconds() / 60)  # 转换为min
respond_time = rides_df_7.loc[rides_df_7["accepted_at"].notnull()].groupby(groupby_status). \
...
# 34 【接驾】
rides_df_7["order_pickup"] = rides_df_7["pickup_to"] - rides_df_7["actual_pickup_at"]
...
pickup_punctual = rides_df_7.\
            ...
            groupby(groupby_status).agg(pickup_punctual=("order_pickup",lambda x:(x >= 0).sum()))      #34 【接驾准时量】
pickup_punctual["os_actual"] = rides_df_7. \
    query(f"actual_dropoff_at.notnull() and manual_assigned_count == 0"). \
    groupby(groupby_status).agg(os_actual=("actual_dropoff_at", "count"))
...
# 司机服务状态
driver_exceed_3_status = rides_df_7.groupby(["route_id","route_name","operation_center_id","operation_center_name","driver_id"]).agg(
...
driver_exceed_3 = driver_exceed_3_status.groupby(groupby_status).agg(
...
# 订单组人数
group_number = rides_df_7.groupby(["route_id","route_name","operation_center_id","operation_center_name","group_id"]).agg(
...
# 接单状态
accepted_status = rides_df_7.query("accepted_at.notnull()").groupby(groupby_status).\
...
accepted_status["accepted_order_backend_A"]=rides_df_7.query("client=='backend' and manual_assigned_count >0 and accepted_at.isnull()").groupby(groupby_status).\
...
columns_list = [null_status, completed_status, order_paytime_mean, order_booking_df, group_number,
                order_booking_completed_df, respond_time, pickup_punctual, driver_exceed_3,accepted_status]
# 合并多个数据表
# for df in columns_list:
week_route_report = pd.concat(columns_list, join="outer", axis=1)  # 把每个表的index都设置相同

"""
合并后的数据表计算
"""
# 5 【接单比例% 】(响应比例)
week_route_report["accepted_order_percent"] = week_route_report["accepted_order"] / week_route_report["order_count"]
# 5 【后台创建接单比例% 】
week_route_report["accepted_order_percent_backend"] = \
    week_route_report["accepted_order_backend"] / week_route_report["order_console"]


# 6 【接驾订单量】（取消时间>时间头 + 总完成订单，还要把已接单的筛出来）
week_route_report[["pickup_count","pickup_backend","pickup_qrcode"]] = rides_df_7. \
...
week_route_report["pickup_count"] = week_route_report["pickup_count"].fillna(0) + week_route_report["order_serving"]
week_route_report["pickup_backend"] = week_route_report["pickup_backend"].fillna(0) + week_route_report["order_console_completed"]  # 后台创建订单接驾量
week_route_report["pickup_qrcode"] = week_route_report["pickup_qrcode"].fillna(0) + week_route_report["order_qrcode_completed"]   # 扫码订单接驾量


# 7 【接驾比例%】
week_route_report["pickup_count_percent"] = week_route_report["pickup_count"] / week_route_report["order_count"]

# 8 【完成率】
week_route_report["order_serving_percent"] = week_route_report["order_serving"] / week_route_report["order_count"]

# 11 【过期比例%】
week_route_report["order_expired_present"] = week_route_report["order_expired"] / week_route_report["order_count"]

# 15 【乘客取消比例%】
week_route_report["order_passenger_canceled_present"] = \
    week_route_report["order_passenger_canceled"] / week_route_report["order_count"]
# 16 【司机取消比例%】
week_route_report["order_driver_canceled_present"] = \
    week_route_report["order_driver_canceled"] / week_route_report["order_count"]
# 17 【后台取消比例%】
week_route_report["order_console_canceled_present"] = \
    week_route_report["order_console_canceled"] / week_route_report["order_count"]

# 22 后台创建比例
week_route_report["order_console_present"] = \
    week_route_report["order_console"] / week_route_report["order_count"]
# 23 后台创建完成率
week_route_report["order_console_completed_present"] = \
    week_route_report["order_console_completed"] / week_route_report["order_console"]
# 24 扫码创建比例
week_route_report["order_qrcode_present"] = \
    week_route_report["order_qrcode"] / week_route_report["order_count"]
# 25 扫码创建完成率
week_route_report["order_qrcode_completed_present"] = \
    week_route_report["order_qrcode_completed"] / week_route_report["order_qrcode"]

# 27 【实时订单量】
week_route_report["order_right"] = week_route_report["order_count"] - week_route_report["order_booking"]
# 27 【实时订单比例%】
week_route_report["order_right_percent"] = week_route_report["order_right"] / week_route_report["order_count"]
# 28 【预约订单比例%】
week_route_report["order_booking_percent"] = week_route_report["order_booking"] / week_route_report["order_count"]
# 30 【实时完成订单量】
week_route_report["order_right_completed"] = week_route_report["order_serving"] - week_route_report[
    "order_booking_completed"]
# 31 【实时订单完成比例%】
week_route_report["order_right_completed_percent"] = week_route_report["order_right_completed"] / \
                                                      week_route_report["order_right"]
# 32 【预约订单完成比例%】
week_route_report["order_booking_completed_percent"] = week_route_report["order_booking_completed"] / \
                                                        week_route_report["order_booking"]

# 36 【人工调单量比例%】
week_route_report["order_assigned_percent"] = week_route_report["order_assigned"] / week_route_report["order_count"]
# 38 【驾驶员改派比例%】
week_route_report["order_driver_reassign_percent"] = week_route_report["order_driver_reassign"] / week_route_report[
    "order_count"]
# 40 【后台改派率%】
week_route_report["order_console_reassign_percent"] = week_route_report["order_console_reassign"] / \
                                                       week_route_report["order_count"]
# 41 【总调度量】
week_route_report["total_operate"] = \
    week_route_report["order_assigned"] + week_route_report["order_driver_reassign"] + week_route_report[
        "order_console_reassign"]
# 42 【总调度率%】
week_route_report["total_operate_percent"] = week_route_report["total_operate"] / week_route_report["order_count"]

# 53 【评价总数】
week_route_report = week_route_report. \
    assign(rating_total=lambda x: x["rating_1"] + x["rating_2"] + x["rating_3"] + x["rating_4"] + x["rating_5"])
# 54 【评价率%】
week_route_report["rating_percent"] = week_route_report["rating_total"] / week_route_report["order_serving"]
# 54 【订单好评率】
week_route_report["rating_best_percent"] = \
    (week_route_report["rating_1"] + week_route_report["rating_2"] + week_route_report["rating_3"]) / \
    week_route_report["rating_total"]

# 59 【上座数/趟】
week_route_report["group_number"] = week_route_report["rider_number_completed"] / week_route_report[
    "rider_group_completed"]
# 60 【每天服务趟数/车】
week_route_report["group_driver_day"] = week_route_report["rider_group_completed"] / week_route_report[
    "driver_exceed_3"] / 7

# 61 【单均价格】
week_route_report["order_average"] = week_route_report["pay_price"] / week_route_report["order_serving"]
# 62 【人均价格】
week_route_report["order_number_average"] = week_route_report["pay_price"] / week_route_report[
...
week_route_report["pickup_time"] = time_total_A["time_A"]*60/time_total_A["time_A_count"]   #【接驾时长（均次）】
week_route_report["distance_time"]  = time_total_A["time_B"]*60/time_total_A["time_B_count"]  #【行程时长（均次）】

# 【预付订单未支付】
week_route_report["not_prepaid"] = rides_df_7.query("is_prepaid ==1 and prepayment_completed == 0 ").groupby(groupby_status).agg(
...
for a, b in zip(arg["name"], arg["c_name"]):
    arg_dict[a] = b
week_route_report.rename(columns=arg_dict, inplace=True)

week_route_report.to_csv(wc.week_route_report_path)
print("******week_report_math_route运行结束******")