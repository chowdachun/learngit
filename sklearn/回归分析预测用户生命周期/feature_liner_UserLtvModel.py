...
#用热力图查看每个特征的相关性
# sns.heatmap(df_uesr.corr(),cmap="YlGnBu", annot = True)
# plt.show()
print("相关性方法查看每个特征与标签的相关关系：",df_uesr.corr())


#使用sklearn中的feature_selection模块中的SelectKBest单变量特征选择工具来选择
Selector = SelectKBest(score_func=mutual_info_regression,k=3)
Selector.fit(x,y)
print(x.columns[Selector.get_support()])
# x_seletor = x.columns[Selector.get_support()]
x_seletor = x.loc[:,Selector.get_support()]
print(x_seletor)
...
x_train_std = std_scale.fit_transform(x_train)    #拟合并转换训练集数据
x_test_std = std_scale.transform(x_test)     #转换测试集
x_vaild_std = std_scale.transform(x_vaild)    #转换验证集
#引用随机森林模型进行
model_rdr_std = RandomForestRegressor()
model_rdr_std.fit(x_train_std,y_train)
print("标准差-随机森林模型分数：",r2_score(y_vaild,model_rdr_std.predict(x_vaild_std)))

#标准化缩放对比-正常数据集的随机森林模型
model_rdr = RandomForestRegressor()
model_rdr.fit(x_train,y_train)
print("没缩放-随机森林模型分数：",r2_score(y_vaild,model_rdr.predict(x_vaild)))
"""
标准化缩放的示例比较-end（有0.0002的下降）
"""
...
x_train_minmax = minmax_scale.fit_transform(x_train)
x_vaild_minmax = minmax_scale.transform(x_vaild)
#引入随机森林模型
model_rdr_minmax = RandomForestRegressor()
model_rdr_minmax.fit(x_train_minmax,y_train)
print("归一化-随机森林模型分数：",r2_score(y_vaild,model_rdr_minmax.predict(x_vaild_minmax)))

# print("归一化数据集",x_train_minmax)
# print("标准差数据集",x_train_std)
# print("正常数据集",x_train)
...
model_rdr_dummies = RandomForestRegressor()
model_rdr_dummies.fit(x_dummies_train,y_dummies_train)
print("增加类别特征client后的vaild-R2：",r2_score(y_dummies_vaild,model_rdr_dummies.predict(x_dummies_vaild)))
