...
class Youhua_logic_model:
    ...
    def add_feature_clean(self, df):  # 增加特征
        ...
        df.drop(columns=["渠道转换", "最近下单天数", "最早下单时间", "最近下单时间", "最早上车时间","最早用券时间"], inplace=True)
    ...
    def back_merge(self, back_test_1_pred=None, pred_0=None, a=0, select_startime=None, select_endtime=None,):
        ...
        back_test_1_pred = pd.merge(back_test_1_pred, pred_0[["下单量"]],      #预测集/实际集
        ...
        back_test_1_pred["最近下单时间"] = pd.to_datetime(back_test_1_pred["最近下单时间"], format='%Y-%m-%d %H:%M:%S', errors='coerce')
        print("内函数时间：",back_test_1_pred["最近下单时间"].min(),back_test_1_pred["最近下单时间"].max())
        back_test_1_pred = back_test_1_pred.query(f"最近下单时间 >= @select_startime and @select_endtime > 最近下单时间 ")
    ...
    def fit(self):
        ...
        for i in range(self.back + 1):  #储存预测结果back+1个df
            ...
            df_pred_first = self.logi_regression\
                (x_train=x_train,y_train=y_train,x_test=x_test,y_test=y_test,x_pred=x_pred,x_pred_first=df_yuan_ls[i])
        ...
        for i in range(self.back):
            #时间过滤
            select_endtime = pd.to_datetime(self.end_time, format='%Y-%m-%d') - dt.timedelta(days=(i+1) * self.lossdays)
...
if __name__ == '__main__':
    ...
    route_id = None
    to_path = "/Users/admin/Desktop/days_60/近30天用户流失预测_20230717.csv"
